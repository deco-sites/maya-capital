      ...
      <main class="w-full">
        <ul class="flex flex-col justify-center items-center gap-4">
          {links?.items?.map((link) => (
            <li class="w-full">
              <a target="_blank" href={link.href} class={linkClass}>
                {Boolean(link.icon) && (
                  <Icon
                    size={20}
                    id={link.icon!}
...
function generateLinkClasses(props?: Props) {
  ...
  return `${txtColor} ${colorClasses} ${hoverColorClasses} group h-[52px] px-6 rounded-full flex justify-start items-center font-bold gap-4`;
}
